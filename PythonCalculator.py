...
#9 Making it so the user doesn't have to enter a space
        calc_parts = re.findall(r'\d+\.\d+|\d+|[+\-^*(rt)]+', calculate.replace(' ', '')    )
        print(calc_parts)
...
#5 Creating subtraction functionality
        if operator == '-':
            print(f'{num1} - {num2} = {num1 - num2}')
#7 Creating other operatior functionality
        if operator == '*':
            print(f'{num1}*{num2} = {num1*num2}')
        if operator == '/':
            print(f'{num1}/{num2} = {num1/num2}')
#8 Creating exp and root functionality
        if operator == '^':
            print(f'{num1}^{num2} = {math.pow(num1, num2)}')
        if operator == 'rt':
            print(f'{num1}root{num2} = {math.pow(num1, 1/num2)}')
